...
//  Natpass_Mac
//
//  Created by etudiant btssnir on 29/03/2022.
//
...
struct Modelcategories: Codable {
    ...
    class ServiceCategories : ObservableObject
    {
        @Published var resultat = Modele()
        let adresse : URL
        init() {
            guard let testUrl = URL(string: "http://172.16.21.32/natpass/categorie.php") else {
                print ("erreur URL !")
